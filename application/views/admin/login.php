...
<body class="login-body">
    <div class="login-card">
        ...
        <div class="login-body-form">
            ...
            <form method="post" action="<?= base_url('admin/do_login') ?>" id="loginForm" autocomplete="on">
                ...
                <div class="form-group">
                    ...
                    <div class="form-input-wrap has-icon">
                        ...
                        <input type="text" name="username" id="username" class="form-control" required autofocus
                            placeholder="Masukkan username" autocomplete="username" autocapitalize="off"
                            autocorrect="off">
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="form-input-wrap has-icon">
                        ...
                        <button type="button" class="icon-right" onclick="togglePassword()" title="Lihat password"
                            id="pwToggle">👁️</button>
                    </div>
                </div>
            ...
            <?php if (ENVIRONMENT === 'development'): ?>
                <div class="demo-hint">
                    ...
                    <span style="font-size:11px;">Atau login pakai akun toko dari Owner</span>
                </div>
            <?php endif; ?>
        </div>

        <div class="login-foot">
            <a href="<?= base_url('admin/lupa_password') ?>">Lupa Password?</a>
            <span class="divider">·</span>
            <a href="<?= base_url('admin/register') ?>" style="color:#ff6b35;font-weight:700;">🏪 Daftar Toko Baru</a>
            <span class="divider">·</span>
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#loginForm').on('submit', function (e) {
                ...
                const alertMsg = $('#login-alert-message');

                // Tampilkan loading state
                btn.prop('disabled', true);
                btnText.html('<span class="spinner"></span> Memproses...');
                alertDiv.hide();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status === 'success') {
                            // Ganti text tombol jadi sukses
                            btnText.text('✅ Login Berhasil!');
                            ...
                            btnText.html('🔓 Masuk ke Dashboard');
                        }
                    },
                    error: function () {
                        // Handle error koneksi AJAX
                        alertMsg.text('Terjadi kesalahan koneksi. Silakan coba lagi.');
                        alertDiv.show();
                        btn.prop('disabled', false);
                        btnText.html('🔓 Masuk ke Dashboard');
                        $('input[name="<?= $this->security->get_csrf_token_name(); ?>"]').val(response.csrf_hash);
        ...
        // Auto-focus & error shake (opsional, bisa disesuaikan)
        const url = new URL(window.location);
        if (url.searchParams.get('err')) {
            document.getElementById('username').focus();
        }

        // Touch feedback for input
