...
.profile-avatar { width: 64px; height: 64px; border-radius: 50%; background: linear-gradient(135deg, var(--tema-color, #ff6b35), var(--tema-color, #ff6b35)); color: #fff; display: flex; align-items: center; justify-content: center; font-size: 26px; font-weight: 700; }
...
<?php $this->load->view('toko/_sidebar', ['toko' => $toko, 'current_page' => 'akun']); ?>
<div class="admin-main">
<?php $this->load->view('toko/_header', ['toko' => $toko, 'current_page' => 'akun', 'page_title' => 'Akun', 'breadcrumb' => 'Pengaturan username & password']); ?>
...
<div class="profile-avatar"><?= strtoupper(substr($toko->pemilik, 0, 1)) ?></div>
<div class="profile-info">
<h3><?= htmlspecialchars($toko->nama_toko) ?></h3>
<p><?= htmlspecialchars($toko->pemilik) ?> · <?= htmlspecialchars($toko->kategori) ?></p>
</div>
</div>
...
<input type="text" name="username" id="akunUsername" class="form-control" required value="<?= htmlspecialchars($toko->username) ?>">
<p class="help-text">Username untuk login ke dashboard toko ini</p>
</div>
</div>
</div>
...
<input type="password" name="password_lama" id="pwLama" class="form-control" placeholder="Kosongkan jika tidak ingin ganti password">
...
<?php $this->load->view('toko/_bottom_nav', ['current_page' => 'akun']); ?>
</div>
</div>
...
document.getElementById('akunForm').addEventListener('submit', function(e) {
    ...
    fetch('<?= base_url('admin/update_akun') ?>', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(data => {
...
function toast(msg, type) { /* moved to admin.js */ }
</script>
